...
class Layer {
   ...
   setCanvasFitScreen() {
      ...
      myCanvas.width = window.innerWidth + (config.translateXMax * 2);
   ...
   class Pencil {
      ...
      init(nbTower = 10) {
         ...
         this.initLayers(config.nbLayers);

         this.initAndDistribTowersOnLayers(nbTower, config.nbLayers);
      }
   ...
   let gui = new dat.GUI();
   //gui.remember(config);
   gui.add(config, "numberOfTowers").min(2).max(100).step(1);
   gui.add(config, "nbLayers").min(2).max(10).step(1);
   gui.add(this, "redraw");
